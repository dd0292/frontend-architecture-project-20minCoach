  ...
  async request<T = any>(
    ...
    if (!this.circuitBreaker.canExecute()) {
      const error = new AppError(
        ...
        { ...context, url, circuitBreakerState: 'OPEN' }
      );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw ErrorAdapter.fromHttpError(response, { ...context, url: fullUrl });
      }
      ...
      throw ErrorAdapter.toAppError(error, { ...context, url: fullUrl });
    }
  }
 ...
 * Convenience methods
 */
  async get<T = any>(url: string, config?: Omit<RequestConfig, 'method'>, context?: ErrorContext): Promise<T> {
    return this.request<T>(url, { ...config, method: 'GET' }, context);
  }

  async post<T = any>(url: string, body?: any, config?: Omit<RequestConfig, 'method' | 'body'>, context?: ErrorContext): Promise<T> {
    return this.request<T>(url, { ...config, method: 'POST', body }, context);
  }

  async put<T = any>(url: string, body?: any, config?: Omit<RequestConfig, 'method' | 'body'>, context?: ErrorContext): Promise<T> {
    return this.request<T>(url, { ...config, method: 'PUT', body }, context);
  }

  async delete<T = any>(url: string, config?: Omit<RequestConfig, 'method'>, context?: ErrorContext): Promise<T> {
    return this.request<T>(url, { ...config, method: 'DELETE' }, context);
  }

  async patch<T = any>(url: string, body?: any, config?: Omit<RequestConfig, 'method' | 'body'>, context?: ErrorContext): Promise<T> {
