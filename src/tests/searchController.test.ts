...
describe("SearchController", () => {
  ...
  describe("validateProblemDescription", () => {
    test("should return valid for description with 40+ words", () => {
      ...
      const result =
        SearchController.validateProblemDescription(longDescription);
    ...
    test("should return invalid for description with less than 40 words", () => {
      const shortDescription =
        "I need help with anxiety and stress management.";

      const result =
        SearchController.validateProblemDescription(shortDescription);
    ...
    test("should handle description with exactly 40 words", () => {
      ...
      const result =
        SearchController.validateProblemDescription(exactly40Words);
    ...
    test("should handle description with multiple spaces between words", () => {
      const descriptionWithMultipleSpaces =
        "I    need    help    with    anxiety    and    stress    management    and    would    like    to    speak    with    a    professional    who    can    help    me    develop    coping    strategies    and    better    manage    my    stress    levels    and    I    am    particularly    interested    in    cognitive    behavioral    therapy    techniques    and    mindfulness    practices";

      const result = SearchController.validateProblemDescription(
        descriptionWithMultipleSpaces,
      );
  ...
  describe("filterCoaches", () => {
    ...
    test("should filter by specialization query", () => {
      const result = SearchController.filterCoaches(
        mockCoaches,
        "programming",
        [],
      );
